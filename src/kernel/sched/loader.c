...
bool elf_load(void *elf_data, uint64_t size) {
    ...
    uint8_t *raw_bytes = (uint8_t *)elf_data;
    printf("ELF Debug: First 16 raw bytes: ");
    for (int k = 0; k < 16; k++) {
        printf("%x ", raw_bytes[k]);
    }
    printf("\n");

    Elf64_Ehdr *ehdr = (Elf64_Ehdr *)elf_data;
    printf("DEBUG [2/7]: ELF Header checked. Entry point: %x, Program headers offset: %x, Count: %u\n", 
    ...
    printf("DEBUG [3/7]: New PML4 address space created at virtual: %x\n", (uint64_t)new_pml4);
    ...
    for (uint16_t i = 0; i < ehdr->e_phnum; i++) {
        if (phdr[i].p_type == 1) { // PT_LOAD segment
            ...
            printf("DEBUG [4/7]: Loading segment %u -> VMA: %x, FileSize: %u, MemSize: %u\n", 
    ...
    printf("DEBUG [5/7]: All PT_LOAD segments successfully mapped and copied.\n");
    ...
    printf("DEBUG [6/7]: User stack allocated and mapped at %x\n", user_stack_top);
    ...
    *--stack = 0;                     // Interrupt number
    ...
    printf("DEBUG [7/7]: Task structure created. Enqueueing to scheduler...\n");
    sched_enqueue(task);
