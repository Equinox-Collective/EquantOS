...
void init_idt(void) {
    ...
    // 2. Set CPU Exceptions (Vectors 0..31) to panic stubs
    for (int i = 0; i < 32; i++) { 
        set_idt_gate(i, isr_stub_table[i], sel); 
    }

    // 3. FIX: Safely route all hardware IRQs (Vectors 32..255) to ignore stub by default
    for (int i = 32; i < 256; i++) {
        set_idt_gate(i, (uint64_t)irq_ignore_handler, sel);
    }

    // 4. Register active hardware IRQs
    // IRQ0 -> PIT Timer (Vector 32)
    set_idt_gate(32, (uint64_t)irq0_handler_asm, sel); 
    idt[32].flags = 0xEE; // Present, DPL 3, 64-bit Interrupt Gate
