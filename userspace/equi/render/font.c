...
// Clean 8x16 Embedded System Font (Standard ASCII 32 - 126)
...
void font_draw_char(surface_t *surf, int x, int y, char c, uint32_t color, int scale) {
    ...
    const uint8_t *glyph = font_8x16_data[(unsigned char)c - 32];
    ...
    for (int cy = 0; cy < FONT_HEIGHT; cy++) {
        ...
        for (int cx = 0; cx < FONT_WIDTH; cx++) {
            if (row & (0x80 >> cx)) {
                if (scale == 1) {
                    draw_pixel(surf, x + cx, y + cy, color);
                } else {
                    draw_fill_rect(surf, x + (cx * scale), y + (cy * scale), scale, scale, color);
                }
            }
        }
    }
...
void font_draw_string(surface_t *surf, int x, int y, const char *str, uint32_t color, int scale) {
    ...
    while (*str) {
        if (*str == '\n') {
            ...
            font_draw_char(surf, cur_x, y, *str, color, scale);
...
int font_get_string_width(const char *str, int scale) {
    if (!str) return 0;
    return (int)strlen(str) * FONT_WIDTH * scale;
}

// 8x16 ASCII Bitmap Table (32..126)
const uint8_t font_8x16_data[95][16] = {
    ...
    {0x00,0x00,0xfe,0xc6,0x06,0x0c,0x18,0x30,0x30,0x60,0x60,0x60,0x00,0x00,0x00,0x00}, // '7'
    ...
    {0x00,0x00,0x7c,0xc6,0xc6,0xde,0xde,0xdc,0xd8,0xc0,0xc6,0x7c,0x00,0x00,0x00,0x00}, // '@'
    ...
    {0x00,0x00,0xfe,0x62,0x62,0x68,0x78,0x68,0x62,0x62,0x62,0xfe,0x00,0x00,0x00,0x00}, // 'E'
    ...
    {0x00,0x00,0xf0,0x60,0x60,0x60,0x60,0x60,0x62,0x62,0x66,0xfe,0x00,0x00,0x00,0x00}, // 'L'
    ...
    {0x00,0x00,0x3c,0x66,0xc3,0xc3,0xc3,0xc3,0xc3,0xdb,0x6e,0x3c,0x06,0x0f,0x00,0x00}, // 'Q'
    {0x00,0x00,0xfc,0x66,0x66,0x66,0x7c,0x6c,0x66,0x66,0x66,0xe6,0x00,0x00,0x00,0x00}, // 'R'
    {0x00,0x00,0x7c,0xc6,0xc0,0x60,0x38,0x0c,0x06,0x06,0xc6,0x7c,0x00,0x00,0x00,0x00}, // 'S'
    ...
    {0x00,0x00,0xc6,0xc6,0xc6,0xc6,0xc6,0x6c,0x6c,0x38,0x38,0x10,0x00,0x00,0x00,0x00}, // 'V'
    {0x00,0x00,0xc3,0xc3,0xc3,0xc3,0xdb,0xdb,0xff,0x66,0x66,0x66,0x00,0x00,0x00,0x00}, // 'W'
    ...
    {0x00,0x00,0x0c,0x0c,0x00,0x1c,0x0c,0x0c,0x0c,0x0c,0x0c,0xcc,0xcc,0x78,0x00,0x00}, // 'j'
    ...
    {0x00,0x00,0x00,0x00,0x00,0xfe,0xdb,0xdb,0xdb,0xdb,0xdb,0xdb,0x00,0x00,0x00,0x00}, // 'm'
    ...
    {0x00,0x00,0x00,0x00,0x00,0xc3,0xc3,0xdb,0xdb,0xff,0x66,0x66,0x00,0x00,0x00,0x00}, // 'w'
